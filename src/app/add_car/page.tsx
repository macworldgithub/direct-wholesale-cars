...
interface FormData {
  ...
  stockNumber?: string;
  bayNumber?: string;
  // regoNumber removed as per request
  vin?: string;
  engineNumber?: string;
...
const CarAuctionForm: React.FC = () => {
  ...
  useEffect(() => {
    if (isEditMode && selectedAd) {
      // Selected ad shape may differ; cast to any for edit prefill
      const ad: any = selectedAd as any;
      reset({
        ...(ad || {}),
        price: ad?.price?.toString(),
        odometer: ad?.odometer?.toString(),
        cyls: ad?.cyls?.toString(),
        seats: ad?.seats?.toString(),
        imageKeys: ad?.images || [],
        branch: ad?.branch as
          | "W - WS VIC"
          | "W - WS QLD"
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (wholesaler?._id) {
        // Wholesaler flow: POST /cars/{wholesalerId}
        const body = {
          branch: data.branch || "",
          ...
          fuelType: data.fuelType || "",
          cylinders: data.cyls ? Number(data.cyls) : undefined,
          transmission: data.transmission || "",
          seats: data.seats ? Number(data.seats) : 0,
          // regoDue removed as per request
          asking: Number(data.price),
          available: data.available ?? true,
        };
        await dispatch(
          createCarForWholesaler({ wholesalerId: wholesaler._id, body })
        ).unwrap();
      } else {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="car-auction-form">
      ...
      <Controller
        ...
        render={({ field }) => (
          <LocalizedInput
            {...field}
            label={isWholesaler ? "Asking" : "Price"}
            required
            type="number"
          />
        )}
      ...
      <Controller
        ...
        render={({ field }) => (
          <LocalizedInput
            {...field}
            label={isWholesaler ? "Stock" : "Stock #"}
            value={field.value ?? ""}
          />
        )}
      />
      ...
      {/* <Controller
        ...
        render={({ field }) => (
          <LocalizedInput
            {...field}
            label={isWholesaler ? "Rego Due" : "Rego #"}
            value={field.value ?? ""}
          />
        )}
      /> */}
